...
function handleDelete() {
  ...
  const isCardList = targetDel.classList.contains("card-list");
  ...
  if (confirmDelete == true) {
    if (isCardList) {
      ...
      removeCardFromLocal(targetDel);
    } else if (isListForm) {
      const cardContentWrapper = targetDel.parentElement;
      removeCurrentTarget(targetDel);
      removeListFromLocal(targetDel);
...
function handlePlusBtn(formPlusCard) {
...
function handleSubmitAddCard(e, formPlusCard) {
  e.preventDefault();
...
function removeListFromLocal(listForm) {
...
function removeCardFromLocal(cardList) {
